...
void InitTetris(){
	...
	timed_out=0;
        ...
        for (int i = 0; i < CHILDREN_MAX; i++){
                ...
                space += sizeof(RecNode)+sizeof(char)*WIDTH*HEIGHT;
        }
...
// 프루닝 트리를 만드는 modified_recommend 이다.
        ...
        for (int tempR = 0; tempR < 4 ; tempR++){
                ...
                for (int tempX = -1 ;tempX <= WIDTH; tempX++){
                        ...
                        // 그렇지 않다면 다음 레벨로 들어간다.
                                ...
                                tempVal = modified_recommend(c[count]);
...
void recommendBlockDown(int sig){
        ...
        if (CheckToMove(field, nextBlock[0], blockRotate, blockY, blockX) == 0 && nowTop){
                gameOver = 1;
                return;
        }
...
long evalSize(RecNode *head){
        double tempSpace = 0;
        if (head->lv == VISIBLE_BLOCKS){
                for (int a = 0; a < CHILDREN_MAX;a++){
                        //free(head->c[a]->f);
                        //free(head->c[a]);
                        tempSpace += sizeof(RecNode) + sizeof(char)*WIDTH*HEIGHT;
                }
                return tempSpace;
        }
        ...
        for (int i= 0; i < CHILDREN_MAX; i++) {
                childSize += evalSize(head->c[i]);
                //free(head->c[i]->f);
                //free(head->c[i]);
        }
                
        return sizeof(RecNode) + childSize;
}
*/
...
void recommendedPlay(){
	...
	do{
		if(timed_out==0){
			alarm(1);
                ...
                // recgetCommand를 이용하여 입력은 q만 받는다.
	...
	printw("GameOver!!");
        ...
        // 시간, 메모리, 시간효율성, 공간효율성을 모두 출력한다.
        move(25, 0);
        printw("t : %lf seconds\n", duration);
        printw("score(t) points: %d\n",score);
        printw("time(t) second: %lf\n", timeT);
        printw("space(t) bytes: %lf\n\n", space);
